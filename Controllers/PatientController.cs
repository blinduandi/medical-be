    ...
    public async Task<IActionResult> AddAllergy(string patientId, [FromBody] CreateAllergyDto allergyDto)
        ...
        public async Task<IActionResult> GetMedicalRecords(string patientId)
        {
            try
            {
                ...
                if (User.IsInRole("Patient") && User.GetUserId() != patientId)
                {
        ...
        /// </summary>
        [HttpGet("{patientId}/medical-records/{recordId}")]
        [Authorize(Roles = "Doctor,Admin,Patient")]
        public async Task<IActionResult> GetMedicalRecord(string patientId, int recordId)
        {
            try
            {
                // If patient role, can only access their own records
                ...
                var medicalRecord = await _context.MedicalRecords
                    .Where(mr => mr.Id == recordId && mr.PatientId == patientId)
                    .Include(mr => mr.Doctor)
                    .Include(mr => mr.Patient)
                    .Select(mr => new MedicalRecordDto
                    {
            ...
            {
                _logger.LogError(ex, "Error getting medical record {RecordId} for patient: {PatientId}", recordId, patientId);
        ...
        [Authorize(Roles = "Doctor,Admin")]
        public async Task<IActionResult> CreateMedicalRecord(string patientId, [FromBody] CreateMedicalRecordDto dto)
        {
            try
            {
                var validationResult = ValidateModel();
                ...
                await _auditService.LogAuditAsync(
                    ...
                    $"Created medical record for patient: {patientId}. Diagnosis: {dto.Diagnosis}",
                    "MedicalRecord",
                    null,
                    Request.GetClientIpAddress()
                );
                ...
                return CreatedAtAction(nameof(GetMedicalRecord), new { patientId, recordId = medicalRecord.Id }, result);
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        [HttpPut("{patientId}/medical-records/{recordId}")]
        [Authorize(Roles = "Doctor,Admin")]
        public async Task<IActionResult> UpdateMedicalRecord(string patientId, int recordId, [FromBody] UpdateMedicalRecordDto dto)
        {
            try
            {
                var validationResult = ValidateModel();
                ...
                await _auditService.LogAuditAsync(
                    ...
                    $"Updated medical record {recordId} for patient: {patientId}",
                    "MedicalRecord",
                    null,
                    Request.GetClientIpAddress()
                );
            ...
            {
                _logger.LogError(ex, "Error updating medical record {RecordId} for patient: {PatientId}", recordId, patientId);
        ...
        /// </summary>
        [HttpDelete("{patientId}/medical-records/{recordId}")]
        [Authorize(Roles = "Admin")]
        public async Task<IActionResult> DeleteMedicalRecord(string patientId, int recordId)
        {
            try
            {
                var medicalRecord = await _context.MedicalRecords
            ...
            {
                _logger.LogError(ex, "Error deleting medical record {RecordId} for patient: {PatientId}", recordId, patientId);
